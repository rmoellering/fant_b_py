...
from models import Game, Team, Player, BatterGame, PitcherGame, FantasyAverage
...
def parse_players(game, stats, lineups, pstats, bstats, game_id, cur_game):
    ...
    for player, data in stats.items():
        ...
        # 4/2/18 Sisco is missing
        elif game_id == 'mlb.g.380402118' and player == 'mlb.p.10155':
            lineups['home_lineup']['all'][player] = 1

        # 4/17/18 Tepera is missing
        elif game_id == 'mlb.g.380417314' and player == 'mlb.p.9959':
            lineups['away_lineup']['P'][player] = {'order': '3'}
            lineups['home_lineup']['all'][player] = 1
            pstats[541] = {'pitches': 26, 'strikes': 19, 'ground': 2, 'fly': 0, 'batters': 5}

        # 6/18/18 ghost players
        elif game_id == 'mlb.g.380618120' \
                and player in ['mlb.p.9100', 'mlb.p.8179', 'mlb.p.10293', 'mlb.p.8641', 'mlb.p.9642', 'mlb.p.9642',
        ...
        elif game_id == 'mlb.g.380908320' and player in ['mlb.p.8030']:
            continue
        ...
        if player in lineups['home_lineup']['P'].keys():
            spot = int(lineups['home_lineup']['P'][player]['order'])
        elif player in lineups['away_lineup']['P'].keys():
            spot = int(lineups['away_lineup']['P'][player]['order'])
        else:
            spot = -1
...
if len(sys.argv) > 1:
    # get max game date
    if sys.argv[1] == 'max':
        for start in session.query(func.max(Game.start)).filter(Game.year==2019):
            exit(start)
...
for row in session.query(FantasyAverage).filter(FantasyAverage.date==cur_date):
    log.info('Found existing row {}, deleting...'.format(cur_date))
    session.delete(row)
    session.commit()

# batting

# select name, team, ab, r + hr + rbi + obp + slg as findex, r, hr, rbi, obp, slg
# from(
# select p.name, t.name AS team, ab, 
#   cast(r / ab as decimal(4, 3)) - 0.138 as r, 
#   cast(hr / ab as decimal(4, 3)) - 0.040 AS hr, 
#   cast(rbi / ab as decimal(4, 3)) - 0.133 AS rbi,
#   cast(obp1 / obp2 as decimal(4, 3)) - 0.322 as obp, 
#   cast((b4 + b3 + b2 + b1) / ab as decimal(4, 3)) - 0.425 AS slg
# from (select bg.player_id, sum(cast(at_bats as decimal)) as ab, sum(cast(runs as decimal)) as r, sum(cast(bg.home_runs as decimal)) as hr, 
#     sum(cast(runs_batted_in as decimal)) as rbi,
#     cast(sum(hits + walks + hbp) as decimal) as obp1,
#     cast(sum(at_bats + walks + hbp + sac_fly) as decimal) as obp2,
#     cast(sum(bg.home_runs) * 4 as decimal) as b4,
#     cast(sum(triples) * 3 as decimal) as b3,
#     cast(sum(doubles) * 2 as decimal) as b2,
#     cast(sum(hits) - (sum(bg.home_runs) + sum(doubles) + sum(triples)) as decimal) as b1
#   from batter_game bg
#     join game g ON g.id = bg.game_id
#   where g.year = 2019
#   group by 1) sq1
#   join player p ON p.id = sq1.player_id
...
q = session.query(
        func.sum(BatterGame.at_bats).label('ab'), 
        func.sum(BatterGame.runs).label('r'),
        func.sum(BatterGame.home_runs).label('hr'),
        func.sum(BatterGame.triples).label('b3'),
        func.sum(BatterGame.doubles).label('b2'),
        func.sum(BatterGame.runs_batted_in).label('rbi'),
        func.sum(BatterGame.hits).label('h'),
        func.sum(BatterGame.walks).label('bb'),
        func.sum(BatterGame.hbp).label('hbp'),
        func.sum(BatterGame.sac_fly).label('sac_fly')). \
    join(Game). \
    filter(Game.year==cur_date.year, Game.start<=cur_date)
...
for row in q:
    ab = Decimal(row.ab)
    fa.runs = Decimal(row.r) / ab
    fa.home_runs = Decimal(row.hr) / ab
    fa.rbi = Decimal(row.rbi) / ab
    fa.obp = Decimal(row.h + row.bb + row.hbp) / Decimal(ab + row.bb + row.hbp + row.sac_fly)
    fa.slg = Decimal((row.hr * 4) + (row.b3 * 3) + (row.b2 * 2) + (row.h - (row.hr + row.b3 + row.b2))) / ab

# pitching

q = session.query(
        func.sum(PitcherGame.win).label('w'), 
        func.sum(PitcherGame.strikeouts).label('k'),
        func.sum(PitcherGame.hold).label('hld'),
        func.sum(PitcherGame.sv).label('sv'),
        func.sum(PitcherGame.earned_runs).label('er'),
        func.sum(PitcherGame.whole_innings).label('wi'),
        func.sum(PitcherGame.partial_innings).label('pi'),
        func.sum(PitcherGame.walks).label('bb'),
        func.sum(PitcherGame.hits).label('h')). \
    join(Game). \
    filter(Game.year==cur_date.year, Game.start<=cur_date)

for row in q:
    ip = Decimal(row.wi) + (Decimal(row.pi) / 3)
    fa.wins = Decimal(row.w) / ip
    fa.strikeouts = Decimal(row.k) / ip
    fa.save_hold = Decimal(row.hld + row.sv) / ip
    fa.era = (Decimal(row.er) / ip) * 9
    fa.whip = (Decimal(row.bb) + Decimal(row.h)) / ip
